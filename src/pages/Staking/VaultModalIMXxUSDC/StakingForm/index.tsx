...
const StakingForm = (props: React.ComponentPropsWithRef<'form'>): JSX.Element => {
  ...
  const owner = account;
  const spender = chainId ? STAKING_REWARD_ADDRESSES[chainId] : undefined;
  const {
  ...
  const approveMutation = useMutation<ContractReceipt, Error, string>(
    async () => {
      ...
      const spender = STAKING_REWARD_ADDRESSES[chainId];
  ...
  const stakeMutation = useMutation<ContractReceipt, Error, string>(
    async (variables: string) => {
      ...
      const tx: ContractTransaction = await stakingRewardsContract.stake(bigStakingAmount);
      return await tx.wait();
    },
    {
      onSuccess: (data, variables) => {
