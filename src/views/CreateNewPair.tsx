  ...
  return (<View>
    <Container className="create-new-pair">
      <Card className="mt-4 p-4">
        <Row className="steps">
          <Col className={stepClassName(1)}><i className="step-number">{ stepSelected > 1 ? ( <FontAwesomeIcon icon={faCheck} size={'sm'} /> ) : (<>1</>) }</i> Insert pair address</Col>
          <Col className={stepClassName(2)}><i className="step-number">{ stepSelected > 2 ? ( <FontAwesomeIcon icon={faCheck} size={'sm'} /> ) : (<>2</>) }</i> First deploy</Col>
          <Col className={stepClassName(3)}><i className="step-number">{ stepSelected > 3 ? ( <FontAwesomeIcon icon={faCheck} size={'sm'} /> ) : (<>3</>) }</i> Second deploy</Col>
          <Col className={stepClassName(4)}><i className="step-number">{ stepSelected > 4 ? ( <FontAwesomeIcon icon={faCheck} size={'sm'} /> ) : (<>4</>) }</i> Third deploy</Col>
          <Col className={stepClassName(5)}><i className="step-number">{ stepSelected > 5 ? ( <FontAwesomeIcon icon={faCheck} size={'sm'} /> ) : (<>5</>) }</i> Initialize pair</Col>
        </Row>
        { stepSelected === 1 && (<div className="form-group">
          <label htmlFor="pairAddress">Insert UniswapV2 pair address:</label>
          <input id="pairAddress" className="form-control" type="text" onChange={(event) => setUniswapV2PairAddress(event.target.value)} autoComplete="off" />
        ...
        { stepSelected === 2 && (<div className="submit-container">
          <InteractionButton name={`Create ${symbol0} pool`} onCall={createBorrowable0} state={createBorrowable0State} />
        </div>) }
        { stepSelected === 3 && (<div className="submit-container">
          <InteractionButton name={`Create ${symbol1} pool`} onCall={createBorrowable1} state={createBorrowable1State} />
        </div>) }
        { stepSelected === 4 && (<div className="submit-container">
          <InteractionButton name={`Create collateral pool`} onCall={createCollateral} state={createCollateralState} />
        </div>) }
        { stepSelected === 5 && (<div className="submit-container">
          <InteractionButton name={`Initialize ${symbol0}-${symbol1}`} onCall={initialize} state={initializeState} />
        </div>) }
        { stepSelected === 6 && (<div className="submit-container">
